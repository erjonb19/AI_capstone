...
def load_models():
    """Download models from GCS and load into memory on startup."""
    global models
    if models:
...
class CandidateProfile(BaseModel):
    ...
    # Phase 2 — Prepare fields (optional — use defaults if not available)
...
def get_risk_tier(probability: float, outcome: str) -> str:
    """Convert probability to Low/Medium/High risk tier."""
    if outcome == "retention":
        # High probability of staying = Low risk
        if probability >= 0.75: return "Low"
        if probability >= 0.50: return "Medium"
        return "High"
    else:
        # High probability of incident/no-promotion = High risk
...
def predict(candidate: CandidateProfile):
    """
    Takes a candidate profile and returns:
    - Retention risk (will they stay 1+ year?)
    - Safety risk (OSHA incident likelihood)
    - Promotion likelihood (within 24 months)
    Each with Low/Medium/High tier and probability score.
    """
    if not models:
        load_models()
