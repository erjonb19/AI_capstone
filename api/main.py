...
def load_models():
    """Download models from GCS and load into memory — called on first request."""
    global models
    if models:
        return
...
# ── Health check — returns immediately, no model loading ──────────────────────
@app.get("/health")
def health():
    return {
        "status": "ok",
...
# ── Prediction endpoint — loads models on first call ──────────────────────────
@app.post("/predict")
